...
#include "ros/ros.h"
#include "pthread.h"
#include "xm_speech/xm_speech_iat.h"
...
bool iat_main(xm_speech::xm_speech_iat::Request  &req, xm_speech::xm_speech_iat::Response  &res)
{
	int			ret						=	MSP_SUCCESS;
	int			upload_on				=	1; //是否上传用户词表
	const char* login_params			=	"appid = 5d5e43d9, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	...
	*
	*/
	const char* session_begin_params	=	"sub = iat, domain = iat, language = us_en, accent = mandarin, sample_rate = 16000, result_type = plain, result_encoding = utf8";
	...
	*/
	...
	run_iat(req.a, session_begin_params); //iflytek02音频内容为“中美数控”；如果上传了用户词表，识别结果为：“中美速控”。
exit:
	printf("按任意键退出 ...\n");
	getchar();
	MSPLogout(); //退出登录
...
{
    ros::init(argc, argv, "iat_sample");
    ros::NodeHandle n;

    ros::ServiceServer service = n.advertiseService("iat",iat_main);
