...
from xm_smach.gpsr_lib import get_pid
...
class GetSignal(State):
    ...
    def execute(self,userdata):
        ...
        try:
            ...
            rospy.logwarn(self.response.target)
            
            if self.response.num == 1:
                return 'succeeded'
...
class SetPose(State):
    ...
    def execute(self , userdata):
        try:
            ...
            userdata.pos_car = Pose(Point(trans[0],trans[1],trans[2]),Quaternion(rot[0],rot[1],rot[2],rot[3]))
            rospy.logwarn(userdata.pos_car)
            return 'succeeded'
...
# '''joints = [-0.0403842670776 -9.9531093426e-05 0.534790602585 -1.24629709114 0.701173381687 -1.56992521621]
# '''

# '''-0.078975582556 0.0313318123249 -1.53573923071 2.09990376212 -1.8856061764 -1.56996521761'''
