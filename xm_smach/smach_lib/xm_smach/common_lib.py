        ...
        try:
            ...
            try:
                ...
                if userdata.arm_ps.header.frame_id is not '':
                    ...
                    pass
                self.arm_ps_.point.x += -0.0  # -0.14 -0.10 store -0.05
                self.arm_ps_.point.y += 0
                self.arm_ps_.point.z += -0.10# 0.23  0.42
...
class ArmCmdPlace(State):
    def __init__(self):
        State.__init__(self, outcomes=[
                       'succeeded', 'aborted'], input_keys=['Object_Num'])

    def execute(self, userdata):

        try:
            joints2 = [-0.0210+userdata.Object_Num *
                       0.02, -0.5521, 1.0385, -0.4835, 0]
            ...
            joints3 = [-2.49428339885e-05, -1.44365561008, -
                       0.0446576103568, 2.18499946594, 0]
...
class Place2(State):
    ...
    def execute(self, userdata):
        ...
        try:
            lifting_controller_level(0)
            rospy.sleep(1)
            joints2 = [0, -1.432, 1.5692, 0.35, 0]
            arm_controller_level(joints2)
            rospy.sleep(3)
            gripper_service_level(True)
            rospy.sleep(2)
...
class PlaceCloth(State):
    ...
    def execute(self, userdata):
        ...
        try:
            joints2 = [0, -0.8454, 1.2662, -0.4352, -1.57]
            lifting_controller_level(0.0789094045758)
            arm_controller_level(joints2)
            rospy.sleep(2)
            gripper_service_level(True)
            rospy.sleep(2)
            joints3 = [-2.49428339885e-05, -1.44365561008, -
                       0.0446576103568, 2.18499946594, -1.57]
            lifting_controller_level(-0.0403053089976)
            arm_controller_level(joints3)
            return 'succeeded'
        except Exception, e:
...
class PickJustfy(State):
    ...
    def execute(self, userdata):
        try:
            if(userdata.name == 'cola' or userdata.name == 'water'):
...
# #        person_y = person_y - (hypot(persox,person_y)-0.2)*sin(angle)
