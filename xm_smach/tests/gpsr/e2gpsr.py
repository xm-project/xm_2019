    ...
    def __init__(self):
        ...
        with self.sm_Find:
            ...
            self.sm_Pos = StateMachine(outcomes=['succeeded','aborted','error'],
                                    input_keys =['target','current_task'])
            with self.sm_Pos:
                StateMachine.add('RUN_IMG',
                                    RunNode_img(),
                                    transitions={'succeeded':'FINDOBJ','aborted':'aborted'})
                StateMachine.add('FINDOBJ',
                                    FindObject_img(),
                                    transitions={'succeeded':'SPEAK','aborted':'FINDOBJ'},
        ...
        self.sm_Guide = StateMachine(outcomes=['succeeded','aborted'],
                                        input_keys=['target','current_task'])
        with self.sm_Guide:
            self.sm_Guide.userdata.sentences = 'pleas follow me'
            StateMachine.add('SPEAK',
                                Speak(),
        ...
        with self.sm_GPSR:
            ...
            StateMachine.add('BACK_DOOR',
                                NavStack(),
                                transitions={'succeeded':'RECEIVE_TASKS','aborted':'BACK_DOOR','error':'error'},
