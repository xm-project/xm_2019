...
import math
import subprocess
class ArmBefore(State):
    def __init__(self):
        State.__init__(self,
                        outcomes={'succeeded'})
    def execute(self,userdata):
        joints = [-2.05639775842e-05,0.94890178215,-1.84626491373,1.95812415689,-1.5699134361]
        lifting_controller_level(-0.040330016206)
        arm_controller_level(joints)
        return 'succeeded'
class Gpsr():
    def __init__(self):
        ...
        #nav tasks  
        self.sm_Nav_grasp = StateMachine(outcomes = ['succeeded','aborted','error'],
                                input_keys = ['target','current_task'])
        with self.sm_Nav_grasp:
            self.sm_Nav_grasp.userdata.pos_xm =Pose()
            self.sm_Nav_grasp.userdata.target_mode =1
            StateMachine.add('GETTARGET',
                                GetTarget(),
                                transitions ={'succeeded':"NAV_GO",'aborted':'aborted','error':'error'},
                                remapping ={'target':'target','current_task':"current_task",'current_target':'pos_xm','mode':'target_mode'})
            ...
            self.Arm_Nav = Concurrence(outcomes=['succeeded','aborted','error'],
                                        default_outcome=['aborted'],
                                        outcome_map={'succeeded':{'Nav_GO':'succeeded'}},
                                        child_termination_cb = 
                                        input_keys = ['pos_xm'])
            with self.Arm_Nav:
                self.Arm_init=StateMachine(outcomes=['succeeded','aborted','error'])
                with self.Arm_init:
                    StateMachine.add('CHECK_ARM',
                                        CBState(self.check_distance,outcomes=['check','arm','error'],input_keys=['pos_xm']),
                                        transitions={'check':'CHECK_ARM','arm':'MOVE_ARM','error':'error'},
                                        remapping={'pos_xm':'pos_xm'})
                    StateMachine.add('MOVE_ARM',
                                        ArmBefore(),
                                        transitions={'succeeded':'succeeded'})

                self.Nav_loop=StateMachine(outcomes=['succeeded','error'],input_keys=['pos_xm'])
                with self.Nav_loop:
                    StateMachine.add('NAV_GO',
                                        NavStack(),
                                        transitions ={'succeeded':'succeeded','aborted':'NAV_GO','error':'error'},
                                        remapping ={'pos_xm':'pos_xm'})
                Concurrence.add('ARM_INIT',self.Arm_init,remapping={'pos_xm':'pos_xm'})
                Concurrence.add('NAV_LOOP',self.Nav_loop,remapping={'pos_xm':'pos_xm'})
            self.sm_Nav.userdata.sentences = 'I arrive here'
            StateMachine.add('SPEAK',
                                Speak(),
                                transitions = {'succeeded':'succeeded','error':'error'},
        ...
        with self.sm_GPSR:
            ...
            StateMachine.add('NAV_GRASP',
                                self.sm_Nav_grasp,
                                remapping ={'target':'target','current_task':'current_task'},
                                transitions={'succeeded':'GET_NEXT_TASK','aborted':'NAV_GRASP'})
    ...
    def nav_child_cb(self,outcome_map):
        if outcome_map['WAIT'] == 'succeeded':
            ...
            print outcome_map
            return False
    def arm_nav_child_cb(self,outcome_map):
        if outcome_map['']
    def check_distance(self,ud):
        try:
            tf_listener = tf.TransformListner()
            tf_listener.waitForTransform('map','base_link',rospy.Time(,rospy.Duration(60.0)))
            (trans,rot) = self.tf_listener.lookupTransform('/map','/base_link',rospy.Time(0))
            xm_x= trans[0]
            xm_y= trans[1]
            target_x=ud.pos_xm.position.x
            target_y=ud.pos_xm.position.y
            if hypot(xm_x-target_x , xm_y-target_y) < 1.0:
                rospy.logwarn('init_arm')
                return 'arm'
            else:
                return 'check'
        except Exception,e:
