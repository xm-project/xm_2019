...
class Gpsr():
    def __init__(self):
        ...
        with self.sm_EnterRoom:
            ...
            self.sm_EnterRoom.userdata.degree = math.pi/3
            StateMachine.add('AROUND' , 
                                TurnDegree() , 
                                remapping = {'degree':'degree'} , 
                                transitions  ={'succeeded' : 'AROUND2' , 'aborted':'AROUND2','error':'error'}
                                )

            self.sm_EnterRoom.userdata.degree = math.pi/3
            StateMachine.add('AROUND2' , 
                                TurnDegree() , 
                                remapping = {'degree':'degree'} , 
        ...
        with self.sm_Find:
            ...
            with self.sm_Pos:
                ...
                with self.CheckTurn:
                    ...
                    Concurrence.add('TestNav',NavStack(),remapping = {'pos_xm':'nav_pos'})
                ...
                StateMachine.add('GET_POS_1',
                                    GetPos(),
                                    remapping ={'target':'target','current_task':'current_task','pose':'pose','mode':'mode_2'},
                ...
                # self.sm_Pos.userdata.mode_3 =3
                # StateMachine.add('GET_POS_2',
                #                     GetPos(),
                #                     remapping ={'target':'target','current_task':'current_task','pose':'pose','mode':'mode_3'},
                #                     transitions={'succeeded':'NAV_FUCK','aborted':'aborted','error':'error'})

                # StateMachine.add('NAV_FUCK',
                #                     NavStack(),
                #                     remapping ={'pos_xm':'pose'},
                #                     transitions ={'succeeded':'FIND_OBJECT_2','aborted':'NAV_FUCK','error':'error'})

                # self.sm_Pos.userdata.object_pos = PointStamped()
                # StateMachine.add('FIND_OBJECT_2',
                #                 FindObject(),
                #                 transitions ={'succeeded':'SPEAK','aborted':'SPEAK','error':'error'},
                #                 remapping ={'name':'name','object_pos':'object_pos'}) 
                ...
                StateMachine.add('SPEAK',
                                    Speak(),
                                    transitions = {'succeeded':'CHECKTURN','error':'error'},
                                    remapping ={'sentences':'sentences'})
                StateMachine.add('CHECKTURN' , self.CheckTurn ,
                                transitions={'turn':'TURN','aborted':'succeeded','error':'error'},
                                remapping={'nav_pos':'nav_pos'})
                StateMachine.add('TURN',PickTurn(),
                                transitions={'succeeded':'NAV','aborted':'TURN'},
                                remapping={'turn_pose':'pose'})
        ...
        with self.sm_Nav:
            ...
            StateMachine.add('CHECKTURN' , self.CheckTurn ,
                                transitions={'turn':'TURN','aborted':'NAV_GO','error':'error'},
                                remapping={'nav_pos':'pos_xm'})
            StateMachine.add('TURN',PickTurn(),
                                transitions={'succeeded':'NAV','aborted':'NAV_GO'},
                                remapping={'turn_pose':'turn_pose'})
            StateMachine.add('NAV',
                                NavStack(),
                                transitions={'succeeded':'NAV_GO','aborted':'NAV','error':'error'},
                                remapping={'pos_xm':'pos_xm'})
        ...
        with self.sm_Pick:
            ...
            self.sm_Pick.userdata.go_counter = 2
            ...
            StateMachine.add('FIND_OBJECT',
                                FindObject(),
                                transitions ={'succeeded':'PICK_JUS2','aborted':'GO_AHEAD','error':'SPEAK'},
                                remapping ={'name':'name','object_pos':'object_pos','objmode':'objmode'})
            
            StateMachine.add('PICK_JUS2' , 
                                PickJustfy(),
                                transitions = {'succeeded':'POS_JUSTFY','error':'error'},
                                remapping = {'name':'name',
                                            'object_pos':'object_pos'})
            StateMachine.add('GO_AHEAD',
                                GoAhead(),
                                transitions = {'succeeded':'POS_COUNTER','aborted':'aborted','error':'error'})

            StateMachine.add('POS_COUNTER' , 
                                PosCounter() ,
                                transitions = {'continue':'FIND_OBJECT' , 'succeeded':'CHECKTURN' , 'aborted':'aborted'},
                                remapping = {'go_counter':'go_counter'})
            ...
            StateMachine.add('FIND_AGAIN',
                                FindObject(),
                                transitions ={'succeeded':'PICK_JUS','aborted':'NAV_TO','error':'error'},
                                remapping ={'name':'name','object_pos':'object_pos','objmode':'objmode'})
            ...
            self.sm_Pick.userdata.speaker = 'speaker'
            StateMachine.add('CHECKTURN', self.CheckTurn,
                             transitions={
                                 'turn': 'TURN', 'aborted': 'succeeded', 'error': 'error'},
                             remapping={'target_name': 'speaker'})
            StateMachine.add('TURN', PickTurn(),
            ...
            StateMachine.add('NAV',
                             ...
                             remapping={'pos_xm': 'pose'})
            self.sm_Pick.userdata.sentences = 'xiao meng can not find thivcngs'
            StateMachine.add('SPEAK',
                                Speak(),
                                transitions={'succeeded':'succeeded','aborted':'aborted','error':'error'})
        ...
        with self.sm_GPSR:
            ...
            StateMachine.add('CHECK_TURN',
                                CheckTurn(),
                                transitions={'succeeded':'GOOUT','continue':'SPEAK_RESTART','error':'error'})
