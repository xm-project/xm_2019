...
class HelpMeCarry():
    def __init__(self):
        ...
        with self.sm_Person:
            ...
            StateMachine.add('GET_PEOPLE_POS',
                             ...
                             transitions={
                                 'invalid': 'SPEAK', 'valid': 'SPEAK', 'preempted': 'aborted'},
            ...
            #                  NavStack(),
            #                  transitions={
            #                      'succeeded': 'SPEAK', 'aborted': 'NAV_PEOPLE', 'error': 'error'},
            #                  remapping={'pos_xm': 'pos_xm'})
        ...
        with self.pick_up:
            self.pick_up.userdata.name = 'Basket'
        ...
        with self.help_me:
            ...
            self.help_me.userdata.go_target = list()
            ...
            StateMachine.add('PUT',
                             self.sm_Place,
                             transitions={'succeeded': 'FIND_PEOPLE_1',
                                          'aborted': 'FIND_PEOPLE_1',
                                          'error':'FIND_PEOPLE_1'})

            StateMachine.add('FIND_PEOPLE_1',
                             self.sm_Person,
                             transitions={'succeeded': 'NAV_CAR',
                                          'aborted': 'NAV_CAR',
                                          'error':'NAV_CAR'})

            # self.help_me.userdata.pos1 = gpsr_target['bedroom']['pos']
            # self.help_me.userdata.pos2 = gpsr_target['diningroom']['pos']
            # self.help_me.userdata.pos3 = gpsr_target['livingroom']['pos']

            # StateMachine.add('NAV_ROOM_1' ,
            #                     NavStack(),
            ...
            #####not finished#####
            # NAV_CAR????????
            StateMachine.add('TURN_GO_CAR',
                             self.TurnNav,
                             transitions={'succeeded': 'NAV_CAR',
                                          'aborted': 'NAV_CAR',
                                          'error': 'error',
                                          'turn': 'TURN2'},
                             remapping={'nav_pos': 'pos_car'})
    ...
    def trace_child_cb(self, outcome_map):
        if outcome_map['STOP'] == 'stop':
            ...
            subprocess.call(
                'xterm -e rosnode kill people_tracking &', shell=True)
            return True
