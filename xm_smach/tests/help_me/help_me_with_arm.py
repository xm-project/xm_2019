...
#            self.xm_Find.userdata.degree = 45.0
        ...
        with self.pick_up:
            ...
            StateMachine.add('FIND_AGAIN',
                                FindObject(),
                                transitions ={'succeeded':'HEHEHE','aborted':'FIND_AGAIN','error':'PICK_OBJ2'},
                                remapping ={'name':'name','object_pos':'object_pos','objmode':'objmode'})
            StateMachine.add('HEHEHE',
                                ArmBeforeBag(),
                                transitions = {'succeeded':'PICK_OBJ'})
        ...
        with self.help_me:
            ...
            self.help_me.userdata.nav_sentences = 'i find you '
            ...
            self.help_me.userdata.pos_xm = gpsr_target['kitchen']['pos']
            self.help_me.userdata.ask_sentences='you can ask me a question'
            self.help_me.userdata.people_condition = {}
            ...
            StateMachine.add('SET_POSITION',
                                SetPose(),
                                transitions={'succeeded':'PICK','aborted':'GET_TARGET'},
                                remapping = {'pos_car':'pos_car'})
            StateMachine.add('GET_TARGET',
                                GetSignal(),
                                transitions = {'succeeded':'PICK','aborted':'GET_TARGET','error':'error'})
            ...
            StateMachine.add('NAV_ROOM',
                                NavStack(),
                                transitions = {'succeeded':'PUT','aborted':'NAV_ROOM','error':'error'},
                                remapping={'pos_xm':'pos_xm'})
            ...
            StateMachine.add('SPEAK',
                                Speak(),
                                transitions={'succeeded':'ANSWER','aborted':'SPEAK','error':'error'},
                                remapping={'sentences':'ask_sentences'})
            StateMachine.add('ANSWER',
                                Anwser(),
                                transitions={'succeeded':'succeeded','aborted':'ANSWER'})
            
            StateMachine.add('FIND',
                                self.xm_Find,
                                transitions = {'succeeded':'SPEAK_NAV','aborted':'aborted','error':'error'})
