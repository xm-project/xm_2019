...
class WhoisWho():
    def __init__(self):
        ...
        with self.WHOISWHO:
            ...
            StateMachine.add('GETPERSON',
                                ...
                                transitions ={'succeeded':'REMEMBER','aborted':"NAV_BACK",'error':'error'}
                                )
            ...
            StateMachine.add('GETTARGET',
                                ...
                                transitions ={'succeeded':'NAV_ROOM','aborted':'ENDTASK','error':'error'})
    ...
    def PDF(self, userdata):
        try:
            img_result = img = np.ones((750,300),dtype=np.uint8)
            img_result = cv2.cvtColor(img_result, cv2.COLOR_GRAY2BGR)
            name_list = userdata.name_list

            number = 0
            for root, dir, files in os.walk('/home/domestic/Vision/Result/people_identify_final'):
                # print(files)
                for file in files:
                    path = "/home/domestic/Vision/Result/people_identify_final/"+file
                    # print(path)
                    pic = cv2.imread(path)
                    # print(pic.shape)
                    img = np.ones((250,300),dtype=np.uint8)
                    ...
                    name = "This is "+ name_list[number]
                    cv2.putText(img, name, (0, pic_cols+30), font, 1, (0,0,0), 2)
                    y = number*250
                    img_result[y:y+250, 0:300] = img
                    number += 1

            cv2.imshow("result", img_result)
            cv2.waitKey(1000)
            cv2.imwrite("/home/domestic/Vision/Result/result.jpg", img_result)
            os.system("convert /home/domestic/Vision/Result/result.jpg /home/domestic/Vision/Result/result.pdf")
            return 'succeeded'
        except Exception,e:
            rospy.logerr(e)
            return 'error'
