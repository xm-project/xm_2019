...
class GPSR():
    def __init__(self):
        ...
        self.FindObj = StateMachine(outcomes=['succeeded', 'aborted', 'error'], input_keys=['target', 'indice', 'targets','table_num'],
                                    output_keys=['target_pos','object_map_point','newik_id'])
        with self.FindObj:
        ...
        self.CallingPeople = StateMachine(outcomes=['succeeded', 'aborted', 'error'])

        with self.CallingPeople:
            self.CallingPeople.userdata.rec = 2.0
            # StateMachine.add('RUNNODE',
            #                  RunNode(),
            #                  transitions={'succeeded': 'WAIT', 'aborted': 'RUNNODE'})  # On simulation , We need to skip RunNode ;So aborted -> Wait Or aborted->RunNode
            # StateMachine.add('WAIT',
            ...
            #                      'succeeded': 'GET_PEOPLE_POS', 'error': 'error'},
            #                  remapping={'rec': 'rec'})
            self.CallingPeople.userdata.pos_xm = Pose()
            StateMachine.add('GET_PEOPLE_POS',
                             FindPeople2(),#.find_people_,
                             transitions={
                                 'invalid': 'NAV_PEOPLE', 'valid': 'TURN1', 'preempted': 'aborted'},
                             remapping={'pos_xm': 'pos_xm'}
                             )
            
            self.CallingPeople.userdata.degree = math.pi/2
            self.CallingPeople.userdata.degree2 = -math.pi
            StateMachine.add('TURN1' , TurnDegree() , 
                                transitions={'succeeded' :'GET_PEOPLE_POS2' ,
                                                'aborted':'GET_PEOPLE_POS2' ,
                                                'preempted':'aborted'},
            ...
            StateMachine.add('GET_PEOPLE_POS3',
                             FindPeople2(),#.find_people_,
                             transitions={
                                 'invalid': 'NAV_PEOPLE', 'valid': 'SPEAK', 'preempted': 'aborted'},
                             remapping={'pos_xm': 'pos_xm'}
                             )
            ...
            self.CallingPeople.userdata.sentences = 'I find you'
            StateMachine.add('SPEAK',
                             SpeakSentence(),
                             transitions={
        ...
        with self.Find:
            ...
            StateMachine.add('PERSON_OR_POS',
                                ...
                                transitions ={'person':'PERSON','position':'POS','error':'error','calling':'CALL'})
            ...
            StateMachine.add('CALL' , self.CallingPeople , transitions = {'succeeded':'succeeded','aborted':'aborted' ,'error':'error'})
        ...
        with self.Pick:
            ...
            StateMachine.add('PICK2', ArmCmdForTf(),
                             transitions={'succeeded': 'succeeded',
                                          'error': 'error',
                                          'aborted': 'aborted'},
                             remapping={'arm_ps': 'object_map_point', 'mode': 'objmode' , 'newik_id':'newik_id'})
            ...
            StateMachine.add('PICK', ArmCmd(),
                             transitions={'succeeded': 'succeeded',
                                          'error': 'error',
                                          'aborted': 'aborted'},
                             remapping={'arm_ps': 'pick_pos', 'mode': 'objmode','object_state':'object_state', 'newik_id':'newik_id'})
        ...
        with self.Talk:
            ...
            StateMachine.add('OPEN_ISR2' , CBState(self.OpenIsr2 , outcomes = ['succeeded' , 'error']) , 
                                            transitions = {'succeeded':'SPEAK',
                                                            'error':'aborted'} )
            StateMachine.add('SPEAK',
                             GeneralAnswer(),
                             transitions={'succeeded': 'OPEN_ISR1', 'aborted': 'OPEN_ISR1'})
            
            StateMachine.add('OPEN_ISR1' , CBState(self.OpenIsr1 , outcomes = ['succeeded' , 'error']),
                                            transitions = {'succeeded':'succeeded',
                                                            'error':'aborted'})
        ...
        with self.handleTask:
            ...
            self.handleTask.userdata.tasksNum = 1
    ...
    def Arm_psJus(self , userdata):
        try:
            if userdata.name == 'ice_tea' :
                ...
                userdata.pick_pos.point.z+=0.03
    ...
    def GenSen(self , userdata):
        try:
            ...
            if  len(ct)>5:
                ...
                userdata.sentences = 'xiao meng team'
            else:
    ...
    def OpenIsr2(self , userdata):
        try:
            pid = get_pid("isr_sample")
            subprocess.call('kill '+str(pid[0]),shell=True)
            rospy.sleep(1.0)
            subprocess.call('gnome-terminal -x bash -c "rosrun xm_speech isr_sample_question;exec bash"&' ,shell = True)
            rospy.sleep(1.0)
    ...
    def OpenIsr1(self , userdata):
        try:
            pid = get_pid("isr_sample_question")
            subprocess.call('kill '+str(pid[0]),shell=True)
            rospy.sleep(1.0)
            subprocess.call('gnome-terminal -x bash -c "rosrun xm_speech isr_sample;exec bash"&' ,shell = True)
            rospy.sleep(1.0)
